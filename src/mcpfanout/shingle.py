...
- k (k-gram size in bytes), default 22, CHOSEN BY THE SWEEP IN docs/CALIBRATION.md (F1.2) and
  ...
  The curve, over 224 concurrent call pairs that share language structure and no information
  (`corpus/negative/calibration.json`) and over the 60 phase A transfers the bench designed to
  be detectable (`corpus/positive/bench-transfers.json`), regenerated by `make ksweep`:

      k    false positives   bench recall   self-match on realistic material
      8          0.5             1.0                   0.9375
      16         0.2768          1.0                   0.5312
      18-21      0.25            1.0                   0.5
      22-40      0.0             1.0                   0.5 down to 0.25
      41-42      0.0             0.9167                0.25
      48+        0.0             0.0                   0.25
  ...
  and a cost of 0.03 in self-match recall over the old value.
  ...
  from 0.94 to 0.25 across this range: at any k, a real fragment shorter than k is invisible.
