...
> 16 of 17 calls and left **zero flows** in the proxy. The packet-level backstop caught it: ten
> connections going straight past. ([threat 19](docs/THREATS.md))
...
> Across all 87 probed schemas: **38%** of tools are attributable from the schema alone, **22%**
> never can be by content, **40%** depend on the value passed. A design rule for any tracing
...
make backstop RUN=example-concurrent    # outbound SYNs per destination: what a proxy misses
...
`v1.0.0` is published on 19 October 2026, when the disclosure window closes, and its DOI is minted
then. Cite the archived release rather than the default branch: take the current one from
