...
**What phase B was waiting on was not a gate rule, it was a finding, and it is now resolved.** The
sequential pass ran and produced destinations no server's documentation declares, so gate rule 7
said stop and flag before publishing anything from it. Both were attributed to a cause by
measurement rather than by reading (`docs/THREATS.md` threat 15: a bare browser through the same
proxy with no MCP server reaches both), neither carried any of our material, and both were
disclosed (`docs/DISCLOSURE-LOG.md`). `make disclosure` on that run now reads `clear`, the
sequential aggregate is committed at `docs/figures/20260919T115452Z-sequential.json`, and the
concurrent pass is unblocked.
...
`docs/figures/20260919T115452Z-sequential.json`, regenerated by
`python -m mcpfanout.cli figures --run runs/20260919T115452Z-sequential`. Gate rule 7 reads
`clear` on it after the two flagged destinations were attributed to a cause and disclosed
(`docs/THREATS.md` threat 15, `docs/DISCLOSURE-LOG.md`). This section is filled from the artifact
and nothing else.

**The denominator first, because every one of the six is a ratio over it.** 26 tool calls across
the 10 pinned servers, 23 of them carrying arguments. **3 calls errored and the reasons are not
...
connection. The first is a server that never ran; the second is a server that ran and refused.
96 flows in all, 89 of them during `driving` and 7 during `handshake`.

| Number | Measured | Read it as |
| --- | --- | --- |
| 1. Outbound connections per tool call | raw p50 **0**, p95 **3**, max **84**; excluding package infrastructure p50 **0**, p95 **2**, max **3** | the gap between the two columns is one server installing a package mid-call (threat 10). 89 of the 96 flows are package infrastructure; 7 more are the launcher's, before any server process existed, and are excluded from every per-call figure by construction |
| 2. Distinct domains per tool call | p50 **0**, p95 **2**, max **3** | the median tool call in this corpus reaches nothing at all |
| 3. Servers propagating `traceparent` | **0 of 10**, and 0 in both protocol segments (3 servers answering `2024-11-05`, 7 answering `2025-11-25`) | uptake of the trace-context convention is zero here, and the segments say the answer is not an artefact of age: no server propagates it, old or current |
| 4. Provenance coverage | context channel **0.0** (0 of 96 flows carried a planted bait fragment); argument channel **2 of 96** flows carried a k-gram of their own call's arguments, both through the request target, 0 through a body | of 3494 observed outbound bytes, 2948 were request target and 546 were body |
| 5. Attribution grades | 89 `UNATTRIBUTED`, 5 `TEMPORAL_ONLY`, 2 `CONTENT_MATCH_UNCONTESTED`, **0 strong** | 0.0 strong attribution is **by construction** and not a finding: `max_active_calls_in_window` is 1, so `CONTENT_UNIQUE` is unreachable. This cell is what the concurrent pass exists to fill |
| 6. Self-hostable third parties | **0.0** over 4 distinct nodes, all `remote_leaf` | 4 nodes is a denominator too small to carry a fraction, and it is published as the count beside it for that reason |

**The finding of this pass is the shape of the zeros, and it is not the one the corpus was built
expecting.** 8 of the 10 servers produced **no call-caused egress at all**: `everything`,
`filesystem`, `memory`, `sequential-thinking`, `git`, `time` are local by design and behaved that
way, `brave-search` never started, and `github` answered its authentication failure without
reaching a host we could observe. Only two servers egressed during a call: `fetch`, to the
requested host and to a package registry mid-call, and `puppeteer`, to the requested host and to
two destinations that belong to the browser it embeds. So numbers 1, 2 and 6 rest on two servers,
and threat 8 is the sentence to read beside them.
...
`CONTENT_MATCH_UNCONTESTED` because exactly one call was in flight, which is the definition of
this condition rather than a result within it.
...
Run `20260919T130847Z-concurrent`, driven 2026-09-19. Committed artifact:
`docs/figures/20260919T130847Z-concurrent.json`, regenerated by
`python -m mcpfanout.cli figures --run runs/20260919T130847Z-concurrent`. Gate rule 7 reads
`clear`. Filled from that artifact and nothing else; the prediction block above is untouched and
`tests/test_phase_b_prediction.py` would fail if it were not.

**The denominator.** 130 calls across the ladder (20 at N = 2, 40 at N = 5, 70 at N = 10), 6
errored, all 6 for the two reasons threat 8 already names: `brave-search` without its key and
`github` without a token. 127 flows.

| Rung | `CONTENT_UNIQUE` | `CONTENT_AMBIGUOUS` | `UNATTRIBUTED` | Discrimination ratio |
| --- | --- | --- | --- | --- |
| N = 2 | 0 | 0 | 8 | undefined, nothing matched |
| N = 5 | **3** | 0 | 7 | **1.0** |
| N = 10 | **4** | 2 | 96 | **0.667** |
| (window 0: launcher) | | | 7 | |

Pooled: 7 `CONTENT_UNIQUE`, 2 `CONTENT_AMBIGUOUS`, 118 `UNATTRIBUTED`, strong-attribution
fraction **0.0551**, all 9 matches through the request target and none through a body.

**B1 is FALSIFIED at N = 5, and the prediction said in advance what would falsify it.** It
predicted the discrimination ratio would be below 1.0 and would fall as N grows. At N = 5 it is
**1.0**, which the block names explicitly as the falsifying outcome. The second half holds: it
falls to 0.667 at N = 10, and the failure mode there is `CONTENT_AMBIGUOUS` (2 flows matching two
in-flight calls each) rather than a wrong pairing. So the mechanism B1 described is real and
arrives later than predicted, on this material.

The honest qualifier, which is larger than the result: **every one of the 9 matches came from one
server.** `fetch` produced all of them. `puppeteer` egressed at N = 2 and matched nothing;
the other eight servers produced no call-caused egress at all. A discrimination ratio computed
over one server's flows is a fact about that server, and B1 was a prediction about real
arguments in general. It is falsified on the evidence available, and the evidence available is
thin enough that the falsification is itself a single-server result. Both halves of that sentence
have to travel together.

**B2 is confirmed.** 118 of 127 flows are `UNATTRIBUTED`, 0.929, so the grade distribution is thin
at the top rather than wrong at the top, exactly as predicted. Of those 118, most are package
infrastructure ineligible by construction (threat 10's server, now at wave scale) and 7 are the
launcher's.
...
carries them only because the aggregate computes all six from any run.

**Threat 16's input is now measurable and is not yet measured.** All 9 matches are target-channel
matches on URLs, which is the `doc_url`-shaped family where the 22-to-28 byte band lives. Whether
those 9 sit inside the band decides threat 16 cheaply, and that is the next measurement, not a
